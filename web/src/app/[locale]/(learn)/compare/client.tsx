...
const COMPARE_EXTRA_TEXT = {
  goal: {
    ...
    en: "After B",
    ja: "B を読み終えた後の到達点",
    vi: "Sau khi học xong B",
  },
  emptyGoal: {
    zh: "该章节的学习目标暂未整理。",
    en: "The learning goal for this chapter has not been filled in yet.",
    ja: "この章の学習目標はまだ整理されていません。",
    vi: "Mục tiêu học tập cho chương này vẫn chưa được bổ sung.",
  },
  diagnosisLabel: {
    zh: "跃迁诊断",
    en: "Jump Diagnosis",
    ja: "ジャンプ診断",
    vi: "Chẩn đoán bước nhảy",
  },
  nextBestLabel: {
    zh: "更稳的读法",
    en: "Safer Reading Move",
    ja: "より安定した読み方",
    vi: "Cách đọc ổn định hơn",
  },
  adjacentTitle: {
    zh: "这是最稳的一步升级",
    en: "This is the safest upgrade step",
    ja: "これは最も安定した1段階の比較です",
    vi: "Đây là bước nâng cấp ổn định nhất",
  },
  adjacentBody: {
    zh: "A 和 B 相相邻，最适合看“系统刚刚多了一条什么分支、一个什么状态容器、为什么现在引入它”。",
    ...
    vi: "A và B nằm kề nhau, đây là cách tốt nhất để thấy chính xác nhánh mới nào đã xuất hiện, trạng thái mới nào được thêm vào và lý do tại sao nó được đưa vào lúc này.",
  },
  adjacentNext: {
    ...
    vi: "Hãy xem luồng thực thi trước, sau đó đến sơ đồ kiến trúc, và cuối cùng mới quyết định có nên xem diff mã nguồn hay không.",
  },
  sameLayerTitle: {
    zh: "这是同阶段内的跳读",
    en: "This is a same-stage skip",
    ja: "これは同一段階内の飛び読みです",
    vi: "Đây là bước nhảy trong cùng một giai đoạn",
  },
  sameLayerBody: {
    ...
    ja: "同じ段階内ではありますが、飛ばした章が概念分離を担っていることが多く、隣接比較より理解リスクは高まります。",
    vi: "Bạn vẫn đang ở trong cùng một giai đoạn khả năng, nhưng các chương bị bỏ qua thường đảm nhận việc 'tách biệt các khái niệm', vì vậy rủi ro khi đọc đã cao hơn rõ rệt so với so sánh các chương kề nhau.",
  },
  sameLayerNext: {
    ...
    vi: "Nếu bắt đầu thấy bối rối, hãy xem lại chương ngay trước B, sau đó xem các tài liệu bổ trợ thay vì cố gắng đọc hiểu ngay sự khác biệt mã nguồn.",
  },
  crossLayerTitle: {
    en: "This is a cross-stage jump",
    ja: "これは段階をまたぐジャンプです",
    vi: "Đây là một bước nhảy vượt giai đoạn",
  },
  crossLayerBody: {
  ...
  crossLayerNext: {
    zh: "先补桥接文档，再用时间线确认阶段切换理由；如果还虚，就先比较 `B` 的前一章 và `B` 本章。",
  ...
  bridgeNudge: {
    ...
    vi: "Tài liệu bổ trợ đáng học nhất trước bước nhảy này",
  },
  quickLabel: {
    en: "One-Click Compare",
    ja: "ワンクリック比較",
    vi: "Lối vào so sánh nhanh",
  },
  quickTitle: {
    en: "Start with these safe comparison moves instead of selecting two chapters every time",
    ja: "毎回2章を手で選ぶ前に、まず安定した比較入口を使う",
    vi: "Sử dụng các lối vào so sánh ổn định này thay vì phải tự chọn hai chương mỗi lần",
  },
  quickBody: {
    zh: "这些按钮优先覆盖最值得反复看的相邻升级 và 阶段切换，适合第一次理解章节边界，也适合读到一半开始混时快速重启。",
    ...
    vi: "Các nút này ưu tiên bao phủ các bước nâng cấp kề nhau và chuyển đổi giai đoạn đáng xem nhất, phù hợp để hiểu ranh giới chương lần đầu hoặc khởi động nhanh khi bắt đầu thấy bối rối.",
  },
  quickPrevious: {
    zh: "直接改成 B 的前一章 -> B",
    en: "Use B's Previous Chapter -> B",
    ja: "B の直前章と B を比べる",
    vi: "So sánh trực tiếp chương trước của B -> B",
  },
  quickPreviousBody: {
    zh: "如果现在这次跳跃太大，先退回 B 的前一章 và B 做相邻对比，会更容易看清这章真正新增了什么。",
    ...
    vi: "Nếu bước nhảy hiện tại quá lớn, hãy so sánh chương ngay trước B với chương B. Đây là cách dễ nhất để thấy rõ những gì chương này thực sự thêm mới.",
  },
} as const;
...
export default function ComparePageClient({ locale }: { locale: string }) {
  ...
  const docA = useMemo(
    () => docs.find((doc) => doc.version === versionA && doc.locale === currentLocale),
    [currentLocale, versionA]
  );
  const docB = useMemo(
    () => docs.find((doc) => doc.version === versionB && doc.locale === currentLocale),
    [currentLocale, versionB]
  );
  ...
  const recommendedBridgeDocs = useMemo(() => {
    ...
    return getBridgeDocDescriptors(versionB as (typeof LEARNING_PATH)[number])
      .map((descriptor) => {
        ...
        return {
          slug: doc.slug,
          title: pickText(currentLocale, descriptor.title),
          summary: pickText(currentLocale, descriptor.summary),
          fallbackLocale: doc.locale !== currentLocale ? doc.locale : null,
  ...
  const guideB = useMemo(() => {
    if (!versionB) return null;
    return (
      getChapterGuide(versionB as (typeof LEARNING_PATH)[number], currentLocale) ??
  ...
  const jumpDiagnosis = useMemo(() => {
    ...
    if (chapterDistance <= 1) {
      return {
        title: pickText(currentLocale, COMPARE_EXTRA_TEXT.adjacentTitle),
        body: pickText(currentLocale, COMPARE_EXTRA_TEXT.adjacentBody),
        next: pickText(currentLocale, COMPARE_EXTRA_TEXT.adjacentNext),
      };
    }

    if (crossLayer) {
      return {
        title: pickText(currentLocale, COMPARE_EXTRA_TEXT.crossLayerTitle),
        body: pickText(currentLocale, COMPARE_EXTRA_TEXT.crossLayerBody),
        next: pickText(currentLocale, COMPARE_EXTRA_TEXT.crossLayerNext),
      };
    }

    return {
      title: pickText(currentLocale, COMPARE_EXTRA_TEXT.sameLayerTitle),
      body: pickText(currentLocale, COMPARE_EXTRA_TEXT.sameLayerBody),
      next: pickText(currentLocale, COMPARE_EXTRA_TEXT.sameLayerNext),
    };
  }, [chapterDistance, infoA, infoB, currentLocale]);
        ...
        <div className="mt-6 space-y-4">
          <div className="rounded-2xl border border-zinc-200/80 bg-white/70 p-4 dark:border-zinc-800/80 dark:bg-zinc-950/60">
            <p className="text-xs uppercase tracking-[0.22em] text-zinc-400">
              {pickText(currentLocale, COMPARE_EXTRA_TEXT.quickLabel)}
            </p>
            <h2 className="mt-2 text-base font-semibold text-zinc-950 dark:text-zinc-50">
              {pickText(currentLocale, COMPARE_EXTRA_TEXT.quickTitle)}
            </h2>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-zinc-600 dark:text-zinc-300">
              {pickText(currentLocale, COMPARE_EXTRA_TEXT.quickBody)}
            </p>
          ...
          {versionB && previousOfB && previousOfB !== versionA && (
            <div className="rounded-2xl border border-amber-200/80 bg-amber-50/70 p-4 dark:border-amber-900/60 dark:bg-amber-950/20">
              ...
              <h2 className="mt-2 text-base font-semibold text-zinc-950 dark:text-zinc-50">
                {pickText(currentLocale, COMPARE_EXTRA_TEXT.quickPrevious)}
              </h2>
              <p className="mt-2 max-w-3xl text-sm leading-6 text-zinc-700 dark:text-zinc-300">
                {pickText(currentLocale, COMPARE_EXTRA_TEXT.quickPreviousBody)}
              </p>
              <div className="mt-4">
                <button
                  type="button"
        ...
        <div className="space-y-8">
          <Card className="overflow-hidden border-zinc-200/80 bg-[radial-gradient(circle_at_top_left,_rgba(16,185,129,0.14),_transparent_30%),radial-gradient(circle_at_bottom_right,_rgba(59,130,246,0.14),_transparent_36%),linear-gradient(160deg,_rgba(255,255,255,0.98),_rgba(244,244,245,0.98))] dark:border-zinc-800/80 dark:bg-[radial-gradient(circle_at_top_left,_rgba(16,185,129,0.16),_transparent_30%),radial-gradient(circle_at_bottom_right,_rgba(59,130,246,0.18),_transparent_36%),linear-gradient(160deg,_rgba(24,24,27,0.98),_rgba(10,10,10,0.98))]">
            ...
            <div className="grid gap-4 xl:grid-cols-4">
              ...
              <div className="rounded-2xl border border-white/70 bg-white/75 p-4 shadow-sm dark:border-zinc-800/80 dark:bg-zinc-900/70">
                <div className="mb-2 flex items-center gap-2 text-sm font-medium text-zinc-700 dark:text-zinc-200">
                  <Wrench size={16} />
                  <span>{pickText(currentLocale, COMPARE_EXTRA_TEXT.goal)}</span>
                </div>
                <p className="text-sm leading-6 text-zinc-600 dark:text-zinc-300">
                  {guideB?.goal ?? pickText(currentLocale, COMPARE_EXTRA_TEXT.emptyGoal)}
                </p>
              </div>
            </div>
          </Card>
          ...
          {jumpDiagnosis && (
            <Card className="overflow-hidden border-zinc-200/80 bg-[linear-gradient(145deg,rgba(255,255,255,0.98),rgba(244,244,245,0.94))] dark:border-zinc-800/80 dark:bg-[linear-gradient(145deg,rgba(24,24,27,0.96),rgba(9,9,11,0.92))]">
              <CardHeader>
                <p className="text-xs uppercase tracking-[0.22em] text-zinc-500 dark:text-zinc-400">
                  {pickText(currentLocale, COMPARE_EXTRA_TEXT.diagnosisLabel)}
                </p>
              ...
              <div className="grid grid-cols-1 gap-4 px-6 pb-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.9fr)]">
                <div className="rounded-2xl border border-zinc-200/80 bg-white/85 p-4 dark:border-zinc-800/80 dark:bg-zinc-950/70">
                  <p className="text-xs uppercase tracking-[0.18em] text-zinc-400">
                    {pickText(currentLocale, COMPARE_EXTRA_TEXT.nextBestLabel)}
                  </p>
                ...
                <div className="rounded-2xl border border-zinc-200/80 bg-white/85 p-4 dark:border-zinc-800/80 dark:bg-zinc-950/70">
                  <p className="text-xs uppercase tracking-[0.18em] text-zinc-400">
                    {pickText(currentLocale, COMPARE_EXTRA_TEXT.bridgeNudge)}
                  </p>
          ...
          {recommendedBridgeDocs.length > 0 && (
            <Card className="overflow-hidden border-zinc-200/80 bg-[linear-gradient(135deg,rgba(255,255,255,0.98),rgba(244,244,245,0.92))] dark:border-zinc-800/80 dark:bg-[linear-gradient(135deg,rgba(24,24,27,0.96),rgba(9,9,11,0.92))]">
              <CardHeader>
                <p className="text-xs uppercase tracking-[0.22em] text-zinc-500 dark:text-zinc-400">
                  {pickText(currentLocale, {
                    vi: "Hỗ trợ bước nhảy",
                  })}
                </p>
                <CardTitle>
                  {pickText(currentLocale, {
                    vi: `Trước khi nhảy từ ${tSession(versionA)} sang ${tSession(versionB)}, hãy xem các sơ đồ này`,
                  })}
                </CardTitle>
                <p className="text-sm leading-6 text-zinc-600 dark:text-zinc-300">
                  {pickText(currentLocale, {
                    vi: "Trang so sánh không chỉ cho bạn biết 'thêm những gì', mà còn cho bạn biết để tiêu hóa bước nhảy này, những bản đồ cấu trúc và cơ chế nào đáng xem nhất trước tiên.",
                  })}
                </p>
              </CardHeader>
                  ...
                  >
                    ...
                    {doc.fallbackLocale && (
                      <p className="mt-3 text-xs text-amber-700 dark:text-amber-300">
                        {pickText(currentLocale, {
                          vi: `Thiếu bản dịch cho ngôn ngữ hiện tại, tự động chuyển sang ${doc.fallbackLocale}`,
                        })}
                      </p>
                    )}
                  </Link>
          ...
          {/* Rest of execution flow, architecture, tool comparison, and source diff sections unchanged */}
          <div>
            <div className="mb-4">
              <h2 className="text-xl font-semibold">
                {pickText(currentLocale, {
                  vi: "So sánh luồng thực thi chính",
                })}
              </h2>
              <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
                {pickText(currentLocale, {
                  vi: "Hãy xem một yêu cầu thay đổi như thế nào giữa hai chương: nhánh mới xuất hiện ở đâu, kết quả nào sẽ quay lại vòng lặp chính, phần nào chỉ là làn phụ hoặc bên ngoài.",
                })}
              </p>
            </div>
